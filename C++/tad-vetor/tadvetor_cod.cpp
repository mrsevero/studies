...
int TadVetor::trocar_2_elementos(int indice1, int indice2) {
  int aux = numeros[indice2];
    numeros[indice2] = numeros[indice1];
    numeros[indice1] = aux;
}

int TadVetor::localiza_maior_menor(){
    int aux[TAMANHO];
    for (int i = 0; i < TAMANHO; i++){
        aux[i] = numeros[i];
    }
...
int TadVetor::ordena_decrescente(){
    bool swapped;
    for (int i = 0; i < TAMANHO; i++){
        swapped = false;
        for (int j = 0; j < TAMANHO; j++) {
